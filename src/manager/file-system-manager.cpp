...
void FileSystemManager::info(){
    ...
    cout << "Nome do volume.....: " << this->superblock->s_volume_name << endl;
    cout << "Tamanho da imagem..: " << (this->superblock->s_blocks_count * BLOCK_SIZE) << " bytes" << endl;
    cout << "Espaço livre.......: " << (usable_free_blocks * BLOCK_SIZE / 1024) << " KiB" << endl;
    cout << "Inodes livres......: " << this->superblock->s_free_inodes_count << endl;
    cout << "Blocos livres......: " << this->superblock->s_free_blocks_count << " blocos" << endl;
    cout << "Tamanho do bloco...: " << BLOCK_SIZE << " bytes" << endl;
    cout << "Tamanho do inode...: " << this->superblock->s_inode_size << " bytes" << endl;
    cout << "Quantidade de grupos: " << group_count << endl;
    cout << "Tamanho do grupo...: " << this->superblock->s_blocks_per_group << " blocos" << endl;
    cout << "Inodes por grupo...: " << this->superblock->s_inodes_per_group << " inodes" << endl;
    cout << "Tamanho da tabela de inodes: " << inode_table_blocks << " blocos" << endl;
}
...
void FileSystemManager::cat(const char *directory_name) {
    ...
    // Use a função já existente para imprimir todo o conteúdo do arquivo (direto, indireto, duplo, triplo)
...
void FileSystemManager::cp(const char *source_path, const char *target_path) {
    ...
    if (bytes_remaining > 0 && file_inode->i_block[12] != 0) {
        ...
        // 7. Copiar blocos indiretos duplos (se houver)
    if (bytes_remaining > 0 && file_inode->i_block[13] != 0) {
        uint32_t double_indirect[256];
        fseek(this->image, file_inode->i_block[13] * block_size, SEEK_SET);
        fread(double_indirect, sizeof(uint32_t), 256, this->image);
    
        for (int i = 0; i < 256 && bytes_remaining > 0; i++) {
            if (double_indirect[i] == 0)
                continue;
            uint32_t indirect[256];
            fseek(this->image, double_indirect[i] * block_size, SEEK_SET);
            fread(indirect, sizeof(uint32_t), 256, this->image);
    
            for (int j = 0; j < 256 && bytes_remaining > 0; j++) {
                if (indirect[j] == 0)
                    continue;
                fseek(this->image, indirect[j] * block_size, SEEK_SET);
                uint32_t to_read = (bytes_remaining < block_size) ? bytes_remaining : block_size;
                fread(block, 1, to_read, this->image);
                out.write(block, to_read);
                bytes_remaining -= to_read;
        ...
        // 8. Copiar blocos indiretos triplos (se houver)
    if (bytes_remaining > 0 && file_inode->i_block[14] != 0) {
        uint32_t triple_indirect[256];
        fseek(this->image, file_inode->i_block[14] * block_size, SEEK_SET);
        fread(triple_indirect, sizeof(uint32_t), 256, this->image);

        for (int i = 0; i < 256 && bytes_remaining > 0; i++) {
            if (triple_indirect[i] == 0)
                continue;
            uint32_t double_indirect[256];
            fseek(this->image, triple_indirect[i] * block_size, SEEK_SET);
            fread(double_indirect, sizeof(uint32_t), 256, this->image);
            ...
            for (int j = 0; j < 256 && bytes_remaining > 0; j++) {
                if (double_indirect[j] == 0)
                    continue;
                uint32_t indirect[256];
                fseek(this->image, double_indirect[j] * block_size, SEEK_SET);
                fread(indirect, sizeof(uint32_t), 256, this->image);

                for (int k = 0; k < 256 && bytes_remaining > 0; k++) {
                    if (indirect[k] == 0)
                        continue;
                    fseek(this->image, indirect[k] * block_size, SEEK_SET);
                    uint32_t to_read = (bytes_remaining < block_size) ? bytes_remaining : block_size;
                    fread(block, 1, to_read, this->image);
                    out.write(block, to_read);
                    bytes_remaining -= to_read;
